...
def test_task_success(mock_parse, mock_fetch_files, mock_file_content, mock_graph_fn, mock_markdown, mock_post, fake_inputs, mock_request):
...
# === ❌ Case 3: fetch_pr_files fails ===
@patch("app.tasks.fetch_pr_files", side_effect=Exception("fetch_pr_files error"))
@patch("app.tasks.parse_repo_url", return_value=("user", "repo"))
def test_fetch_pr_files_error(mock_parse, mock_fetch, fake_inputs, mock_request):
    result = run_wrapped_task(mock_request, fake_inputs["repo_url"], fake_inputs["pr_number"], fake_inputs["github_token"])
    assert result["status"] == "failed"
    assert "fetch_pr_files error" in result["error"]


# === ❌ Case 4: fetch_file_content fails ===
@patch("app.tasks.fetch_file_content", side_effect=Exception("file content error"))
@patch("app.tasks.fetch_pr_files", return_value=[{"filename": "file1.py"}])
@patch("app.tasks.parse_repo_url", return_value=("user", "repo"))
def test_fetch_file_content_error(mock_parse, mock_files, mock_content, fake_inputs, mock_request):
    result = run_wrapped_task(mock_request, fake_inputs["repo_url"], fake_inputs["pr_number"], fake_inputs["github_token"])
    assert result["status"] == "failed"
    assert "file content error" in result["error"]


# === ❌ Case 5: build_graph fails ===
@patch("app.tasks.build_graph", side_effect=Exception("graph build error"))
@patch("app.tasks.fetch_file_content", return_value="code")
@patch("app.tasks.fetch_pr_files", return_value=[{"filename": "file1.py"}])
@patch("app.tasks.parse_repo_url", return_value=("user", "repo"))
def test_build_graph_error(mock_parse, mock_files, mock_content, mock_build, fake_inputs, mock_request):
    result = run_wrapped_task(mock_request, fake_inputs["repo_url"], fake_inputs["pr_number"], fake_inputs["github_token"])
    assert result["status"] == "failed"
    assert "graph build error" in result["error"]


# === ❌ Case 6: graph.invoke fails ===
@patch("app.tasks.build_graph")
@patch("app.tasks.fetch_file_content", return_value="code")
@patch("app.tasks.fetch_pr_files", return_value=[{"filename": "file1.py"}])
@patch("app.tasks.parse_repo_url", return_value=("user", "repo"))
def test_graph_invoke_error(mock_parse, mock_files, mock_content, mock_build, fake_inputs, mock_request):
    mock_graph = MagicMock()
...
# === ❌ Case 7: markdown generation fails ===
@patch("app.tasks.generate_github_markdown_review", side_effect=Exception("markdown error"))
@patch("app.tasks.build_graph")
@patch("app.tasks.fetch_file_content", return_value="code")
@patch("app.tasks.fetch_pr_files", return_value=[{"filename": "file1.py"}])
@patch("app.tasks.parse_repo_url", return_value=("user", "repo"))
def test_markdown_gen_fails(mock_parse, mock_files, mock_content, mock_build, mock_md, fake_inputs, mock_request):
    mock_graph = MagicMock()
...
@patch("app.tasks.fetch_pr_files", return_value=[{"filename": "file1.py"}])
@patch("app.tasks.parse_repo_url", return_value=("user", "repo"))
def test_post_comment_fail(mock_parse, mock_files, mock_content, mock_build, mock_md, mock_post, fake_inputs, mock_request):
    mock_graph = MagicMock()
    mock_graph.invoke.return_value = {"results": [{"filename": "file.py", "issues": []}]}
    mock_build.return_value = (mock_graph, {})
